...
SELECT rental_date, year(rental_date), month(rental_date), day(rental_date),
hour(rental_date), minute(rental_date), second(rental_date) FROM sakila.rental
WHERE rental_id = 10330;
-- 4 - Monte uma query que retorne todos os dados do pagamento feito no dia 28/07/2005 a partir das 22 horas.
SELECT * FROM sakila.payment
WHERE DATE(payment_date) = '2005-07-28' AND HOUR(payment_date) >= 22; -- Olhei no gabarito essa
